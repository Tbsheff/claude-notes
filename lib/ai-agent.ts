...
Your goal is to help the user modify their codebase by reading, understanding, and editing files as needed.`
  ...
  private async editFile(params: { filepath: string, oldContent: string, newContent: string }): Promise<string> {
  ...
  private async grepSearch(params: { pattern: string, filepath?: string, recursive?: boolean }): Promise<string> {
  ...
  private async findFiles(params: { pattern: string, directory?: string }): Promise<string> {
  ...
  private async listFiles(params: { directory?: string }): Promise<string> {
  ...
  private async finish(params: { message: string }): Promise<string> {
    console.log(`🏁 Finished: ${params.message}`)
    this.isFinished = true
    return params.message
  }

  public async processRequest(userMessage: string): Promise<string> {
  ...
  private async callOpenRouter(): Promise<any> {
    const requestBody = {
